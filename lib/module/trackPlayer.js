...
let androidAutoCallbacks = undefined;
/**
...
global.__rntpAAGetChildrenAsync ??= async parentId => {
  if (androidAutoCallbacks) {
    return androidAutoCallbacks.getChildren(parentId);
  }
  return [];
};
global.__rntpAAGetItemAsync ??= async mediaId => {
  if (androidAutoCallbacks) {
    return androidAutoCallbacks.getItem(mediaId);
  }
  return null;
};
global.__rntpAAGetTrackAsync ??= async mediaId => {
  if (androidAutoCallbacks) {
...
/* eslint-disable  @typescript-eslint/no-explicit-any */
global.__rntpNativeCallJs ??= (fnName, arg, onResolve, onReject) => {
